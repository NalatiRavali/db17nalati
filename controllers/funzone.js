const funzone = require('../models/funzone');
var Costume = require('../models/funzone'); 
...
${JSON.stringify(req.body)}`) 
    try { 
        let toUpdate = await funzone.findById( req.params.id) 
...
failed`); 
    ...
    try{ 
        theCostumes = await Costume.find(); 
        res.render('funzone', { title: 'Costume Search Results', results: theCostumes }); 
    } 
    catch(err){ 
        res.status(500); 
        res.send(`{"error": ${err}}`); 
